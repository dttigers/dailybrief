 ...
 * Callers should use `gte(createdAt, start)` and `lt(createdAt, end)`.
 *
 * When `now` is exactly Wednesday 00:00:00.000 in `tz`, `start === now`.
 *
...
/**
 * Returns a rolling window covering the last `days` full calendar days (aligned
 * to 00:00 in `tz`) through `now`.
 *
 * Semantics: start is INCLUSIVE (00:00 on the anchor day), end === now (not
 * day-aligned). Callers use `gte(createdAt, start)` and `lt(createdAt, end)`.
 *
 * @param tz   IANA timezone string. Throws RangeError if invalid.
 * @param days Non-negative integer number of days. Throws RangeError if invalid.
...
export function getRollingDayWindow(
  ...
  // Approximate the start day by subtracting days * 86400s in UTC.
  // This approximation may be off by an hour at DST transitions but the
  // wall-clock resolution below corrects for that.
  const startApprox = new Date(now.getTime() - days * 86_400_000);
