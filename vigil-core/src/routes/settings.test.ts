...
test("GS-02: PUT valid schedule returns { ok: true }; onUpsert captures values; GET returns updated", async () => {
  ...
  assert.equal(stored!.hour, 5, "onUpsert captured hour");
  assert.equal(stored!.minute, 15, "onUpsert captured minute");
  assert.equal(stored!.enabled, true, "onUpsert captured enabled");
