...
- **D-13:** (added 2026-04-08 after research surfaced the DATABASE_URL gap) The local vigil-core runs in **`status: "degraded"` as the accepted steady state** because Mac apps talk to `api.vigilhub.io` (Railway), not localhost. Bootstrap's health check therefore only checks HTTP 200 on `/v1/health` — it does NOT parse the JSON body for `status == "ok"`. Local vigil-core is kept around for dev tinkering; it is NOT on the critical path for Mac app functionality. `dailybrief-doctor.sh` adds a separate informational row — `local vigil-core DB connected? [yes/no]` — that is NOT a failure condition (doctor's pass/fail is scoped to secret drift, not local DB state). Fixing the DATABASE_URL gap is out of scope for this phase; deferred.
- **D-14:** (added 2026-04-08 POST-SHIP during first real cross-machine setup attempt) **D-01 made a wrong assumption — the user does NOT use 1Password and never did.** Phase 57's discuss-phase presented 1Password CLI as "recommended" and locked it without confirming the user was already a 1Password customer or willing to become one. The assumption was never tested because `op` wasn't installed on the development machine at execute time (executor correctly skipped the happy path, only validated the preflight-fail path). The real first cross-machine setup (2026-04-08, iMac → MacBook Pro at 192.168.1.136) revealed the gap: bootstrap.sh is unrunnable as-shipped for this user. **Pragmatic workaround for the immediate setup:** direct `scp` from the iMac to the MacBook over LAN — no new tools, no subscriptions, works in one session. **Trigger to re-open Phase 57 or file a follow-up phase:** the user signs up for 1Password (at which point bootstrap.sh works as designed), OR the user acquires a third Mac (at which point scp-from-source-machine breaks down and a real answer is needed). **Lesson for future discuss-phase sessions:** when a decision depends on the user having an existing account/subscription/tool, confirm that fact in the AskUserQuestion itself, not just "recommended vs alternatives". The bootstrap/doctor infrastructure is still valuable — `scripts/dailybrief-doctor.sh`, `scripts/install.sh`, and `scripts/sync-anthropic-key.sh` all remain load-bearing; only the `op document get` calls in bootstrap.sh Task 2 are currently unusable.
