...
- [ ] **AGENT-WATCH-02**: Daemon parses each new JSONL line, emits the 5 Vigil event types per detection rules (`needs_input` debounced 30s/session, `task_failed` deduped per session, `milestone` once per pattern per session, `heartbeat` after ≥60s silence, `task_complete`/`task_failed` precedence rule), and persists per-file byte offsets to `~/Library/Application Support/vigil-watch/offsets.json` so daemon restarts don't replay history.
- [ ] **AGENT-WATCH-03**: Daemon POSTs events to `${api_url}/v1/agent-events` with bearer auth, retry/backoff, in-memory queue (max 100) for offline buffering, and 5-second drain on `SIGTERM`.
