...
---
*Last updated: 2026-05-01 — Phase 118 complete (OPS-01 satisfied). Two test users (`upper@case.com` id=3 + `test+phase104@local.test` id=44) and 22 cascaded child rows (10 briefs + 10 brief_pdfs + 2 users; other 11 user-scoped tables already empty for these ids) deleted from Railway prod via idempotent dry-run-by-default `vigil-core/scripts/cleanup-test-users.ts` (350 lines, 14-table single-tx with pre-flight email assertion gate). Audit trail: `118-RUN-LOG.txt` (verbatim stdout, both DRY-RUN and COMMIT sections + TRANSACTION COMMITTED banner) + `118-RUNBOOK.md` (Before/After table all After=0, Smoke-Pass PASS, Rollback Notes). Smoke confirmed seed user (`jamesonmorrill1@gmail.com` id=1, 607 thoughts/21 briefs/15 brief_pdfs) untouched — SC#4 evidenced via login + /v1/auth/me + /v1/thoughts total=194 + PWA dashboard load. Two documented blocking-class plan deviations auto-fixed: (1) Railway internal-only DATABASE_URL → corrected to `--service Postgres + DATABASE_PUBLIC_URL` form (D-01 invariant preserved), (2) PIPESTATUS scoping bug in tee'd pipeline → commit exit code inferred 0 from TRANSACTION COMMITTED banner + all-zero AFTER SELECTs. Code review 0C/1W/4I (advisory only — connection-pool teardown via process.exit, unused TARGET_IDS const, schema-drift footgun). Phase verification PASSED 4/4. Incidental security finding flagged: Railway Postgres password leaked to agent context during URL discovery (NOT to disk) — rotation recommended as defense-in-depth follow-up. v3.7 milestone now 5/6 phases complete; only Phase 119 (DMARC quarantine ramp) remains, gated on SEED-003 routine fire 2026-05-06. Backlog 999.2 (CaptureBar paste-side newline strip) and SEED-004 (token rotation copy) still queued.*
