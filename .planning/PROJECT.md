...
---
*Last updated: 2026-04-30 — Phase 116.1 complete (SPORTS-01b sports route + PWA error-class differentiation), automated 3/3 SCs PASS, UAT-01 (countdown UI) verified live, UAT-02 (BALLDONTLIE_API_KEY env-gap repro) + UAT-03 (brief PDF placeholder) tracked as pending in HUMAN-UAT. WR-01 (unencoded teamId) and code-review WR-01/WR-02 (catastrophic-rejection PostHog gap + regex-parse fragility) all closed in this phase via `LeagueResult.errorKind` structured field. v3.7 milestone now 3/5 phases complete; remaining: 117 (auth-email rate-limit UX), 118 (prod test-user cleanup), 119 (DMARC quarantine ramp). Backlog 999.2 still queued: CaptureBar paste-side newline strip (POLISH-01 D-16). Token rotation copy explicitly deferred as SEED-004.*
