...
**Phase 113 complete (2026-04-26, code shipped to Railway; HUMAN-UAT pending):** Verify email on signup — `users.email_verified_at TIMESTAMPTZ` added in Drizzle migration 0017 with backfill UPDATE that grandfathered all 117 existing users to `created_at` (Plan 01, idempotent SQL applied via `npm run db:migrate`). Server surface: `auth.ts` register handler issues 24h `email_verify` token + fires `sendEmailVerificationEmail` via `.catch()` (Plan 02 — both fresh-register and claim branches, with `existing.emailVerifiedAt IS NULL` guard for the seed-user claim path). New `auth-me.ts` ships `GET /v1/auth/me` returning minimal `{ id, email, emailVerifiedAt }` (intentionally distinct path from existing `/v1/me` which has different shape — RESEARCH finding #1). Login response extended with `emailVerifiedAt` (Plan 02 D-26). `verify-email.ts` (Plan 03) ships unauth `POST /v1/auth/verify-email` with atomic `UPDATE … RETURNING` claim filtered by `type='email_verify'` + 5/hr per-IP inline rate-limit + single-bucket 400 error. `resend-verification.ts` (Plan 03) ships bearerAuth-protected `POST /v1/auth/resend-verification` with already-verified short-circuit (D-18) + 3/hr per-userId limit (SC#5) + most-recent-link-wins token invalidation (D-17). PWA: `VerifyEmailPage.tsx` (Plan 04) at `/auth/verify` with **0 useEffect calls** — Apple Mail prefetch defense load-bearing — uses raw `fetch()` not `vigilFetch` to dodge the Authorization header + 401 handler interference (D-19, RESEARCH finding #3); 4 states (initial Confirm / success in-place swap with no redirect per SC#3 / generic single-bucket error / explicit malformed-link state). `SettingsPage.tsx` (Plan 04) renders banner only when `/v1/auth/me` returns `emailVerifiedAt === null`, non-dismissible, with 5-state Resend lifecycle (Idle → Sending… → Sent! 10s → Idle, plus 429 inline + 5xx inline) using project's custom `bg-warning-50/border-warning-400/text-warning-400` Tailwind v4 @theme tokens. Live e2e smoke (`smoke-test-verify-email.ts`) walks register → DB-token-read → verify → assert email_verified_at + replay → assert 400, with `finally`-block cleanup so re-runs leave no orphans. **Pending:** 5 SCs need Railway-prod confirmation per `113-HUMAN-UAT.md` (Gmail inbox arrival, Apple Mail prefetch test, banner-clears-on-reload, prod backfill grandfathering, resend 429). 29 commits auto-pushed to origin/main on phase close → Railway redeploy in progress. AUTH-11 satisfied at code level.
...
- [◐] Verify email on signup (AUTH-11 — Phase 113, code shipped, HUMAN-UAT pending)
...
---
*Last updated: 2026-04-26 — Phase 113 code complete (HUMAN-UAT pending). AUTH-11 shipped at code level: Drizzle 0017 backfill grandfathered all 117 existing users, register-handler email_verify token issuance + fire-and-forget send, POST /v1/auth/verify-email atomic claim, POST /v1/auth/resend-verification with bearerAuth + 3/hr/user limit + already-verified short-circuit, GET /v1/auth/me minimal {id,email,emailVerifiedAt}, PWA VerifyEmailPage with 0 useEffect calls (Apple Mail prefetch defense) + Settings banner with 5-state Resend lifecycle. 29 commits auto-pushed to origin/main → Railway redeploy in progress; 5 SCs require live UAT against prod (113-HUMAN-UAT.md status: partial). v3.6 progress: 6/7 phases complete; only Phase 114 (Safari extension EXT-02) remains.*
