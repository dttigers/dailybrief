...
---
*Last updated: 2026-04-29 — Phase 116 complete (SPORTS-01 sports source picker), live UAT 4/4 SCs + 6/6 human items PASS after local `BALLDONTLIE_API_KEY` env-gap fix. v3.7 milestone now 2/5 phases complete; remaining: 117 (auth-email rate-limit UX), 118 (prod test-user cleanup), 119 (DMARC quarantine ramp). Phase 116.1 queued for gap closure (route try/catch + PWA error-class differentiation — see code review WR-01..WR-04 in 116-REVIEW.md). Backlog 999.2 captured: CaptureBar `<input>` strips paste-side newlines — out of scope for POLISH-01 D-16, surfaced during Phase 115 UAT. Token rotation copy (rotated vs expired) explicitly deferred — captured as SEED-004 follow-up.*
