# Requirements Archive: v3.7 Source Pickers, Verify-Email UX & Closeout Cleanup
...
- [x] **OPS-02**: SEED-003 DMARC ramp `p=none → p=quarantine` on `vigilhub.io` Cloudflare DNS, gated on the auto-eval routine that fires 2026-05-06 (≥7 days clean aggregate reports + ≥3 days verify-email production volume). Phase implementation lands the runbook + ramp action; ramp action only fires after gate passes. **Closed via Phase 119 operator amendment 2026-05-06** — gate fired and returned DEFERRED (0/3 conditions met due to vigilhub.io's pre-growth email volume); operator amendment accepted `p=none` as steady-state DMARC posture and documented three explicit re-activation conditions (volume materializes / spoofing observed / compliance requirement). Cloudflare DNS untouched; gate routine `trig_01RZLcj1jpxvDQAwnFmUG9d9` preserved for opportunistic re-firing.
...
| OPS-02 | Phase 119 | Complete (deferred via amendment) |
