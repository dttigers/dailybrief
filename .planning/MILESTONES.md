...
## v3.8 Claude Code Companion (Shipped: 2026-05-11)

**Delivered:** Even Realities G2 glasses now function as an ambient notification + status layer for long-running Claude Code sessions. A locally-built Swift daemon (`vigil-watch`) observes `~/.claude/projects/` via FSEventStream, parses 5 Vigil event types from Claude Code JSONL traces, and posts to Vigil Core; the G2 plugin's new Companion HUD renders a 3-line glanceable view fed by per-userId SSE fan-out. Plus a mid-milestone wide-release auth hardening insert (Phase 126) that took Vigil from family-allowlist beta to public-traffic-ready in a single day.

**Phases completed:** 7 phases (120-126; Phase 126 was a mid-milestone insert added 2026-05-11), 54 plans
**Timeline:** 5 days active execution (2026-05-07 → 2026-05-11)
**Requirements:** 28/28 (20 original + 8 AUTH-126 added mid-milestone)
**Git range:** `docs(120-01)` (08613cfe, 2026-05-07) → `chore(state): Phase 126 SHIPPED to prod` (b138c475, 2026-05-11) — 200 commits, 245 files changed, +64,890 / −284 LOC
**Production:** vigil-core + PWA live on Railway throughout; G2 plugin `vigil.ehpk` v0.3.0 (30,564 bytes) resubmitted to Even Hub developer portal store dashboard 2026-05-11
**Known deferred items at close:** 19 (see STATE.md Deferred Items) — 13 dormant seeds (most carry to v3.9), 2 backlog phases (999.1, 999.2), 2 stale quick-task metadata, 1 audit false-positive (knowledge-base index), 1 risk-accepted SC (Phase 123 post-reboot resume — OS guarantee + plist-pinned + drift-detected, flips on next natural reboot)

**Key accomplishments:**

- **Day-1 JSONL schema verification gate (Phase 120, VERIFY-01)** — Empirical capture of Claude Code's JSONL schema via two passes: (1) corpus mining of 184 existing JSONL files spanning 30 days of natural usage on iMac, (2) a 33-line scripted session covering 5 D-02 scenarios with verbatim line-range mapping and reproducible excerpt evidence. Synthesized into a publicly-readable Day-1 findings document at `github.com/dttigers/vigil-watch` (commit 5273534). Verdict `spec-correct-and-proceed`, 8-row observed mapping, 4 prose answers with JSONPath references, 8 hand-curated sanitized excerpts. Three documented fallback paths (notification observation / VS Code extension / process inspection) stayed untouched. Public MIT SPM repo with 7-header README + secret-hygiene `.gitignore` preempting T-120-01 user-content leak.
- **Per-userId agent-events API foundation (Phase 121, AGENT-API-01, AGENT-API-02)** — Postgres `agent_events` table with 11-column schema, CHECK on event (5 values), composite partial unique index for cross-user-safe idempotency, Drizzle-inferred types. `POST /v1/agent-events` (idempotent via composite partial unique on `(user_id, client_event_id)`) + `GET /v1/agent-sessions` (sliding 24h, DISTINCT ON CTE) as a Hono sub-app with factory DI, manual strict validation, and sliding-window session list. 24-test route suite + 15-test isolation suite covering W-01/W-02-style cross-user isolation lock blocks pinning POST userId-from-bearer invariant, GET per-user scoping, and composite dedupe scope. Two pre-existing bugs fixed in passing (TS 5.9 narrowing + Postgres partial-index ON CONFLICT mismatch).
- **`vigil-watch` core daemon (Phase 122, AGENT-WATCH-01/02/03/06)** — Swift 6 strict-concurrency-safe FSEventStream bridge (CallbackBox/Unmanaged) + WatcherActor tail-cursor pipeline reading new bytes from stored offsets, buffering partial lines, routing through `parseJSONLLine`. Stateless `parseJSONLLine` implementing Phase 120's 8-row mapping table verbatim (including 4 level-shift corrections), 11 XCTest cases against fixture corpus. NSRegularExpression compile-once milestone matcher actor (D-10 implicit `(?i)` + opt-out) with `(sessionId, pattern)` dedupe (D-07) and priorEmissions hydration for restart-survival (D-08). Actor `StateStore` owning `offsets.json` exclusively with D-09 schema_version=2 + F_FULLFSYNC + rename atomic write. Per-session timer state machine actor with all 5 detection rules (task_failed/task_complete precedence, needs_input debounce, heartbeat re-arm). EmitterActor with locked retry/backoff/queue/drain semantics + URLSession DI behind HTTPClient protocol. Hand-rolled flat TOML parser with bracket-depth array termination, WatchConfig defaults for all D-03/D-04/D-05/D-10 locked values. Two-stream logging surface (stdout NDJSON + stderr text with bearer masking). 106 total XCTest cases green.
- **`vigil-watch` shell + 24h soak (Phase 123, AGENT-WATCH-04/05/07)** — `swift-argument-parser` 1.6.0 wired with `VigilWatchCLI` `@main` dispatcher and 6 typed `AsyncParsableCommand` subcommands (`run`/`tail`/`test`/`install`/`uninstall`/`status`). Daemon emits an atomic snake_case JSON snapshot to `~/Library/Application Support/vigil-watch/runtime-state.json` once per second (D-04/D-05). Idempotent bootout-then-bootstrap install with chmod 0600 plists + `<true/>` self-closing booleans + xmlEscape on api key (T-123-01/02/03 mitigations). SIGINT-forwarding `tail | jq` pipeline. Synthetic `_vigil_test_<unix>` POST round-trip. Three-state Status output (RUNNING/NOT RUNNING/NOT INSTALLED) via runtime-state.json freshness + launchctl print fallback. `scripts/soak-check.sh` enforcing all 5 D-09 gates verbatim, 5 SoakCheckTests pinning every failure mode against synthetic CSV fixtures. **24h SOAK PASSED** on 2026-05-10 CSV: max RSS 7220 KB (24% of 30 MB budget), unique PIDs 1 (KeepAlive held), uptime span 86128s (23h 55m), 288 samples. Empty `watch.err` across the full window confirms zero empty-session_id parser drops.
- **G2 Companion HUD + per-userId SSE fan-out (Phase 124, AGENT-API-03, AGENT-HUD-01/02, G2-POLISH-06/07)** — `Map<userId, EventEmitter>` bus singleton in `vigil-core/src/lib/agent-events-bus.ts` with structural cross-user isolation, auto-cleanup on last-listener unsubscribe, `setMaxListeners(50)` headroom for reconnect storms. Per-userId SSE fan-out at `GET /v1/agent-stream` via Hono `streamSSE` — Last-Event-ID resume bounded to last 24h, lazy bus subscribe inside the streamSSE callback closure, 25s keepalive (.unref()'d), `onAbort` cleanup pairs `bus.off` with `clearInterval`. Hand-rolled fetch + ReadableStream + TextDecoder SSE shim for the G2 plugin (no `EventSource` available) with bearer in Authorization header (NEVER URL), Last-Event-ID resume from `localStorage['vigil:lastEventId']`, exact `[1000, 2000, 4000, 8000, 16000, 30000]` exponential backoff (D-11), AbortController disconnect, `event: ping` keepalive frames silently dropped, `safeWriteStorage` try/catch for QuotaExceededError survival. 3-line glanceable HUD (label / state / last event). Phase 124 D-08 narrowed SC #2 from three-tap promise (single/double/long) to context-sensitive DOUBLE_CLICK overload (banner-ack → cycle-session → jump-home) after empirical G2 hardware test — single-tap (`CLICK_EVENT`) and long-press (`LONG_PRESS_EVENT`) not reliably plumbed on G2 hardware, deferred to SEED-011. G2-POLISH-06 (launch-source registration) + G2-POLISH-07 (210px home overflow with 4-line invariant drift detector) folded in. Reconnect storm + operator decision tree captured in 124-VERIFICATION.md.
- **Quiet mode + plugin v0.3.0 ship + polish riders (Phase 125, AGENT-HUD-03, G2-POLISH-05/08, G2-PLUGIN-01, AGENT-DEMO-01)** — Two-column schema delta (`users.quiet_mode boolean default false` + `users.quiet_mode_since timestamptz`) + idempotent migration 0019. SSE fan-out honors quiet_mode (only `needs_input` + `task_failed` reach HUD when on); auto-detect of iPhone Focus/DND deferred to SEED-015 as follow-on UX. G2-POLISH-05 (DOUBLE_CLICK → home documented exit gesture with on-screen hint) + G2-POLISH-08 (device-status `connectType:"none"` debounce). `vigil.ehpk` v0.3.0 packed at 30,564 bytes and resubmitted to the Even Hub developer portal store dashboard. 5-doc amendment cascade (single-tap → double-tap, swipe → documented exit, SDK quiet-mode pipeline reality). 60s portfolio demo recorded.
- **Wide-release auth hardening — mid-milestone insert (Phase 126, AUTH-126-01..08)** — Triggered 2026-05-11 by a family-signup error triage exposing the auth surface gaps blocking a wide-release flip. Cloudflare Turnstile server-side verify in `vigil-core/src/lib/turnstile.ts` (with DI seam) before allowlist check. Per-IP `RATE_LIMIT_MAX = 5` on `POST /auth/register` reusing Phase 117 drift-detector convention. `require-verified-email` middleware with 24h grace gating `/v1/*`. Sentry server wrapper (`@sentry/node`) + PWA client (`@sentry/react`) with init before `createRoot().render()` and `Sentry.captureException` as additive sibling alongside preserved PostHog call in `ErrorBoundary.componentDidCatch`. D-04 locked-enum `ERROR_CODE_MAP` + `resolveApiError` at `vigil-pwa/src/lib/api-error-codes.ts` with 9 LOCKED keys + 4 extension codes — kills the "Invalid email or password" collapse class structurally. Termly Privacy + Terms at `app.vigilhub.io/legal/{privacy,terms}` with footer + signup-screen links. Anthropic console monthly spend cap set to $500/mo. `VIGIL_ALLOWED_EMAILS="*"` sentinel handling in `isAllowlistedEmail()` as kill-switch — flipped to `*` on 2026-05-11 after 01-07 went green. Live smoke from `app.vigilhub.com` confirmed real Turnstile widget + token validation + signup success. **SECURITY 46/46 threats closed; UAT 8 passed / 0 issues.**

**Patterns established (full retrospective in `.planning/RETROSPECTIVE.md`):**

- Day-1 verification gate for external-system assumptions before downstream production code
- Operator-driven wallclock checkpoints exempt from `skip_checkpoints: true` (24h soak, Mac reboot, dashboard action)
- Liveness ≠ functionality probe — RSS/uptime/PID checks must be paired with synthetic-input → expected-output probes
- `withCheckedContinuation` for top-level Swift daemon lifetime in `AsyncParsableCommand`
- Hand-roll over polyfill when runtime is constrained (G2 plugin SSE shim ~150 LOC vs 10KB polyfill)
- Hardware-truth-first capability narrowing (Phase 124 D-08 three-tap → DOUBLE_CLICK overload)
- Mid-milestone insert for shipping-blocker phases (Phase 126 added + shipped same day)
- Locked-enum API error UX mapping (server `code` field + PWA lookup table structurally kills collapse bugs)
- Risk-accepted closure for physical-host-bound SC items with OS guarantee + plist-pinned + drift-detected fallback

**Regression caught + fixed during the milestone:**

- **R-123-01** — `RunLoop.main.run()` lifted from Phase 122's `main.swift` into `func run() async throws` was a no-op (`AsyncParsableCommand` runs on the cooperative pool, not main thread). Daemon exited clean (exit 0), launchd cycled spawn-scheduled indefinitely, sampler CSV stopped after ~35 min. Fix: `await withCheckedContinuation { _ in }` suspends the Task forever; FSEvents + DispatchSource handlers run on independent queues. Soak window restarted 2026-05-09 ~14:05 PDT. Coverage gap: in-process unit tests never invoked `Run().run()` (would block forever); subprocess-style integration test queued for follow-up phase.
- **vigil-watch silent-but-running** — Daemon passed liveness checks for 28h+ while emitting zero events. FSEvent pipeline silently died at bootstrap exit due to Swift release-mode lifetime collapse of FSEventStream local + UnsafeBitCast UB in FSEventBridge. Resolved via debug session `vigil-watch-silent-no-events` (2026-05-10). Phase 123 Plan 05 operator runbook extended with synthetic-JSONL-append + 10s event-emission probe at soak start AND soak end.
