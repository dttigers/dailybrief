...
export default function AudioUploadSection() {
  ...
  if (phase === 'idle') {
    return (
      <div>
        <h1 className="text-xl font-semibold text-slate-100 mb-6">Upload Audio</h1>
        <label
          htmlFor="audio-input"
        ...
        >
          ...
          <p className="text-slate-300 font-medium mb-1">Upload an audio recording</p>
          <p className="text-slate-500 text-sm">WAV, MP3, M4A, WebM — max 10 MB</p>
          <input
            ref={fileInputRef}
            id="audio-input"
            type="file"
      ...
      <div>
        ...
        <div className="bg-green-900/30 border border-green-700/40 rounded-xl px-6 py-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-green-300 font-semibold">Thought saved</p>
          </div>
          <div className="bg-slate-800 p-3 rounded-lg mb-4">
            <p className="text-slate-200 text-sm">{result.transcription}</p>
          </div>
          <button
            onClick={handleReset}
